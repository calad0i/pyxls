...
extern "C"
{
    ...
    struct xls_dslx_invocation_rewrite_rule {
        ...
        // Optional filter: only invocations whose callee-side ParametricEnv
        // equals this value will be replaced. If null, matches all
        // instantiations.
        const struct xls_dslx_parametric_env *match_callee_env; // optional
        // Optional explicit env for the replacement callee. If null, retains
        // the original invocation's explicit parametrics. If non-null but
        // empty, emits no explicit parametrics (rely on deduction).
    ...
    bool xls_dslx_parametric_env_create(
        const struct xls_dslx_parametric_env_item *items,
        size_t items_count,
        char **error_out,
    ...
    struct xls_dslx_parametric_env *
    xls_dslx_parametric_env_clone(const struct xls_dslx_parametric_env *env);
    ...
    uint64_t
    xls_dslx_parametric_env_hash(const struct xls_dslx_parametric_env *env);

    // Returns a newly allocated string describing the parametric environment.
    char *
    xls_dslx_parametric_env_to_string(const struct xls_dslx_parametric_env *env);
    ...
    struct xls_dslx_interp_value *
    xls_dslx_interp_value_make_ubits(int64_t bit_count, uint64_t value);
    struct xls_dslx_interp_value *
    xls_dslx_interp_value_make_sbits(int64_t bit_count, int64_t value);

    // Constructs an enum InterpValue given an enum def and the underlying bits.
    bool xls_dslx_interp_value_make_enum(
        struct xls_dslx_enum_def *def,
        bool is_signed,
    ...
    bool xls_dslx_interp_value_make_tuple(
        ...
        struct xls_dslx_interp_value **elements,
        char **error_out,
        struct xls_dslx_interp_value **result_out
    );
    ...
    struct xls_dslx_interp_value *
    xls_dslx_interp_value_clone(const struct xls_dslx_interp_value *value);
    ...
    bool xls_dslx_typechecked_module_clone_removing_functions(
        ...
        struct xls_dslx_function *functions[],
    ...
    bool xls_dslx_typechecked_module_clone_removing_members(
        struct xls_dslx_typechecked_module *tm,
        struct xls_dslx_module_member *members[],
        size_t member_count,
        const char *install_subject,
        struct xls_dslx_import_data *import_data,
        char **error_out,
    ...
    void
    xls_dslx_typechecked_module_free(struct xls_dslx_typechecked_module *tm);

    struct xls_dslx_module *
    xls_dslx_typechecked_module_get_module(struct xls_dslx_typechecked_module *);
    ...
    struct xls_dslx_module_member *
    xls_dslx_module_get_member(struct xls_dslx_module *, int64_t);

    xls_dslx_module_member_kind
    xls_dslx_module_member_get_kind(struct xls_dslx_module_member *);

    struct xls_dslx_constant_def *
    xls_dslx_module_member_get_constant_def(struct xls_dslx_module_member *);

    struct xls_dslx_struct_def *
    xls_dslx_module_member_get_struct_def(struct xls_dslx_module_member *);

    struct xls_dslx_enum_def *
    xls_dslx_module_member_get_enum_def(struct xls_dslx_module_member *);

    struct xls_dslx_type_alias *
    xls_dslx_module_member_get_type_alias(struct xls_dslx_module_member *);

    // Returns the function AST node from the given module member if it is a
    // function; otherwise returns nullptr.
    struct xls_dslx_function *
    xls_dslx_module_member_get_function(struct xls_dslx_module_member *);

    // Returns the QuickCheck AST node from the given module member. The caller
    // should ensure the module member kind is
    // `xls_dslx_module_member_kind_quick_check`.
    struct xls_dslx_quickcheck *
    xls_dslx_module_member_get_quickcheck(struct xls_dslx_module_member *);
    ...
    struct xls_dslx_module_member *
    xls_dslx_module_member_from_enum_def(struct xls_dslx_enum_def *enum_def);
    ...
    struct xls_dslx_module_member *
    xls_dslx_module_member_from_function(struct xls_dslx_function *function);
    ...
    // function.
    int64_t
    xls_dslx_function_get_parametric_binding_count(struct xls_dslx_function *fn);

    // Returns the i-th parameter of the given DSLX function.
    // The returned pointer is borrowed and tied to the lifetime of the
    // underlying function/module objects.
    struct xls_dslx_param *
    xls_dslx_function_get_param(struct xls_dslx_function *fn, int64_t index);
    ...
    // DSLX source.
    struct xls_dslx_type_annotation *
    xls_dslx_param_get_type_annotation(struct xls_dslx_param *p);

    // Returns the function body as an expression (statement block).
    struct xls_dslx_expr *
    xls_dslx_function_get_body(struct xls_dslx_function *fn);

    // Returns the return-type annotation for the given function, or nullptr if
    // none was written in the source.
    struct xls_dslx_type_annotation *
    xls_dslx_function_get_return_type(struct xls_dslx_function *fn);
    ...
    // underlying function/module objects.
    struct xls_dslx_attribute *
    xls_dslx_function_get_attribute(struct xls_dslx_function *fn, int64_t index);

    // Returns the attribute kind.
    xls_dslx_attribute_kind
    xls_dslx_attribute_get_kind(struct xls_dslx_attribute *attribute);

    // Returns the number of arguments carried by the attribute.
    int64_t
    xls_dslx_attribute_get_argument_count(struct xls_dslx_attribute *attribute);
    ...
    // Specializes each requested function within `typechecked_module`,
    // producing a cloned module that is re-typechecked and installed into
    // `import_data` under `install_subject`. On success, `*result_out` receives
    // the new module.
    ...
    struct xls_dslx_function *
    xls_dslx_quickcheck_get_function(struct xls_dslx_quickcheck *);
    ...
    bool xls_dslx_quickcheck_get_count(
        struct xls_dslx_quickcheck *,
        int64_t *result_out
    );
    ...
    int64_t
    xls_dslx_module_get_type_definition_count(struct xls_dslx_module *module);
    ...
    xls_dslx_type_defintion_get_colon_ref(struct xls_dslx_type_definition *);
    struct xls_dslx_type_alias *
    xls_dslx_type_definition_get_type_alias(struct xls_dslx_type_definition *);
    ...
    struct xls_dslx_expr *
    xls_dslx_constant_def_get_value(struct xls_dslx_constant_def *);
    ...
    xls_dslx_struct_def_get_member(struct xls_dslx_struct_def *, int64_t);

    // Note: return value is owned by the caller, free via `xls_c_str_free`.
    char *xls_dslx_struct_member_get_name(struct xls_dslx_struct_member *);

    struct xls_dslx_type_annotation *
    xls_dslx_struct_member_get_type(struct xls_dslx_struct_member *);
    ...
    xls_dslx_enum_def_get_member(struct xls_dslx_enum_def *, int64_t);

    struct xls_dslx_type_annotation *
    xls_dslx_enum_def_get_underlying(struct xls_dslx_enum_def *);

    // Note: return value is owned by the caller, free via `xls_c_str_free`.
    char *xls_dslx_enum_member_get_name(struct xls_dslx_enum_member *);

    struct xls_dslx_expr *
    xls_dslx_enum_member_get_value(struct xls_dslx_enum_member *);
    ...
    struct xls_dslx_module *
    xls_dslx_expr_get_owner_module(struct xls_dslx_expr *expr);
    ...
    struct xls_dslx_type_definition *
    xls_dslx_type_ref_get_type_definition(struct xls_dslx_type_ref *);

    // -- type_definition

    struct xls_dslx_colon_ref *
    xls_dslx_type_definition_get_colon_ref(struct xls_dslx_type_definition *);
    ...
    xls_dslx_colon_ref_resolve_import_subject(struct xls_dslx_colon_ref *);
    ...
    struct xls_dslx_type_annotation *
    xls_dslx_type_alias_get_type_annotation(struct xls_dslx_type_alias *);
    ...
    bool xls_dslx_interp_value_convert_to_ir(
        const struct xls_dslx_interp_value *v,
        char **error_out,
    ...
    // Note: if there is no type information available for the given entity
    // these may return null; however, if type checking has completed
    // successfully this should not occur in practice.
    ...
    struct xls_dslx_function *
    xls_dslx_invocation_data_get_callee(struct xls_dslx_invocation_data *data);

    struct xls_dslx_function *
    xls_dslx_invocation_data_get_caller(struct xls_dslx_invocation_data *data);

    // -- type (deduced type information)

    bool xls_dslx_type_get_total_bit_count(
        const struct xls_dslx_type *,
        char **error_out,
        int64_t *result_out
    );

    // Returns whether the given type is a bits-like type with signedness 'true'.
    bool xls_dslx_type_is_signed_bits(
        const struct xls_dslx_type *,
        char **error_out,
        bool *result_out
    );

    bool xls_dslx_type_to_string(
        const struct xls_dslx_type *,
        char **error_out,
        char **result_out
    );

    // Note: on success the caller owns `is_signed` and `size` and must free
    // them via `xls_dslx_type_dim_free`.
    bool xls_dslx_type_is_bits_like(
        struct xls_dslx_type *,
    ...
    struct xls_dslx_enum_def *
    xls_dslx_type_get_enum_def(struct xls_dslx_type *);

    // Precondition: xls_dslx_type_is_struct
    struct xls_dslx_struct_def *
    xls_dslx_type_get_struct_def(struct xls_dslx_type *);

    // Precondition: xls_dslx_type_is_array
    struct xls_dslx_type *
    xls_dslx_type_array_get_element_type(struct xls_dslx_type *);

    // Note: returned xls_dslx_type_dim is owned by the caller and must be
    // deallocated.
    struct xls_dslx_type_dim *
    xls_dslx_type_array_get_size(struct xls_dslx_type *);

    // -- type_dim (deduced type information)

    // TODO: https://github.com/google/xls/issues/3661 - remove this when
    // downstream code stops using it.
    [[deprecated(
        "A TypeDim can never be parametric anymore; this returns false."
    )]]
    ...
    bool xls_dslx_type_dim_get_as_bool(
        struct xls_dslx_type_dim *,
        char **error_out,
        bool *result_out
    );

    bool xls_dslx_type_dim_get_as_int64(
        struct xls_dslx_type_dim *,
        char **error_out,
        int64_t *result_out
    );
    ...
    struct xls_dslx_type *
    xls_dslx_type_array_get_element_type(struct xls_dslx_type *type);

    struct xls_dslx_type_dim *
    xls_dslx_type_array_get_size(struct xls_dslx_type *type);

    // Rewrites invocations in the given module according to the rules. Returns
    // a new typechecked module installed in the import graph under
    // `install_subject`. On error, returns false and populates error_out (owned
    // by caller).
    bool xls_dslx_replace_invocations_in_module(
        struct xls_dslx_typechecked_module *tm,
        struct xls_dslx_function *const callers[],
        size_t callers_count,
        const struct xls_dslx_invocation_rewrite_rule *rules,
        size_t rules_count,
        struct xls_dslx_import_data *import_data,
        const char *install_subject,
        char **error_out,
