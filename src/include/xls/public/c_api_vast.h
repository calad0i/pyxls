...
extern "C"
{
    ...
    // Note: We define the enum with a fixed width integer type for clarity of
    // the exposed ABI.
    ...
    struct xls_vast_verilog_file *
    xls_vast_make_verilog_file(xls_vast_file_type file_type);
    ...
    struct xls_vast_data_type *
    xls_vast_verilog_file_make_scalar_type(struct xls_vast_verilog_file *f);
    ...
    xls_vast_verilog_file_make_bit_vector_type_with_expression(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *expression,
        bool is_signed
    );
    ...
    struct xls_vast_data_type *xls_vast_verilog_file_make_packed_array_type(
        struct xls_vast_verilog_file *f,
        xls_vast_data_type *element_type,
        const int64_t *packed_dims,
        size_t packed_dims_count
    );

    // Creates an unpacked array type.
    //
    // Example (SystemVerilog):
    //   element_type = bit_vector_type(8) and unpacked_dims = {2, 3} yields a
    //   type that emits like: `[7:0] <ident>[2][3]`.
    struct xls_vast_data_type *xls_vast_verilog_file_make_unpacked_array_type(
        struct xls_vast_verilog_file *f,
        xls_vast_data_type *element_type,
        const int64_t *unpacked_dims,
        size_t unpacked_dims_count
    );
    ...
    void xls_vast_verilog_module_add_member_instantiation(
        ...
        struct xls_vast_instantiation *member
    );
    ...
    void xls_vast_verilog_module_add_member_blank_line(
        struct xls_vast_verilog_module *m,
        struct xls_vast_blank_line *blank
    );
    ...
    struct xls_vast_generate_loop *xls_vast_verilog_module_add_generate_loop(
        ...
        struct xls_vast_expression *init,
        struct xls_vast_expression *limit,
        const char *label
    );
    ...
    xls_vast_def *
    xls_vast_verilog_module_port_get_def(struct xls_vast_module_port *port);
    ...
    bool xls_vast_data_type_width_as_int64(
        struct xls_vast_data_type *type,
        int64_t *out_width,
        char **error_out
    );

    bool xls_vast_data_type_flat_bit_count_as_int64(
        struct xls_vast_data_type *type,
    ...
    xls_vast_expression *
    xls_vast_data_type_width(struct xls_vast_data_type *type);
    ...
    // Creates a generic definition (Def) with the given name, data kind, and
    // type. Useful for creating typed parameters via
    // xls_vast_verilog_module_add_parameter_with_def.
    struct xls_vast_def *xls_vast_verilog_file_make_def(
        struct xls_vast_verilog_file *f,
        const char *name,
        xls_vast_data_kind kind,
        struct xls_vast_data_type *type
    );

    // Adds a module parameter with an explicit Def (type/kind) and RHS
    // expression. Returns a handle to the created parameter reference.
    struct xls_vast_parameter_ref *
    xls_vast_verilog_module_add_parameter_with_def(
        struct xls_vast_verilog_module *m,
        struct xls_vast_def *def,
        struct xls_vast_expression *rhs
    );
    struct xls_vast_localparam_ref *
    ...
    void xls_vast_verilog_module_free_ports(
        struct xls_vast_module_port **ports,
        size_t count
    );
    ...
    xls_vast_verilog_file_make_continuous_assignment(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *lhs,
        struct xls_vast_expression *rhs
    );
    ...
    struct xls_vast_blank_line *
    xls_vast_verilog_file_make_blank_line(struct xls_vast_verilog_file *f);
    ...
    struct xls_vast_macro_ref *xls_vast_verilog_file_make_macro_ref_with_args(
        struct xls_vast_verilog_file *f,
        const char *name,
        struct xls_vast_expression **args,
        size_t arg_count
    );

    // Cast: MacroRef -> Expression
    struct xls_vast_expression *
    xls_vast_macro_ref_as_expression(struct xls_vast_macro_ref *ref);
    ...
    struct xls_vast_instantiation *xls_vast_verilog_file_make_instantiation(
        ...
        const char **parameter_port_names,
        struct xls_vast_expression **parameter_expressions,
    ...
    void xls_vast_verilog_file_add_include(
        struct xls_vast_verilog_file *f,
        const char *path
    );
    ...
    void xls_vast_verilog_file_add_comment(
        struct xls_vast_verilog_file *f,
        const char *text
    );
    ...
    struct xls_vast_concat *xls_vast_verilog_file_make_concat(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression **elements,
    ...
    struct xls_vast_slice *xls_vast_verilog_file_make_slice(
        struct xls_vast_verilog_file *f,
        struct xls_vast_indexable_expression *subject,
        struct xls_vast_expression *hi,
        struct xls_vast_expression *lo
    );

    struct xls_vast_expression *xls_vast_verilog_file_make_unary(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *arg,
    ...
    struct xls_vast_expression *xls_vast_verilog_file_make_ternary(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *cond,
    ...
    struct xls_vast_expression *xls_vast_verilog_file_make_width_cast(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *width,
    ...
    bool xls_vast_verilog_file_make_literal(
        struct xls_vast_verilog_file *f,
    ...
    struct xls_vast_expression *
    xls_vast_literal_as_expression(struct xls_vast_literal *v);
    struct xls_vast_expression *
    xls_vast_logic_ref_as_expression(struct xls_vast_logic_ref *v);
    struct xls_vast_expression *
    xls_vast_slice_as_expression(struct xls_vast_slice *v);
    struct xls_vast_expression *
    xls_vast_concat_as_expression(struct xls_vast_concat *v);
    struct xls_vast_expression *
    xls_vast_index_as_expression(struct xls_vast_index *v);
    struct xls_vast_expression *
    xls_vast_parameter_ref_as_expression(struct xls_vast_parameter_ref *v);
    struct xls_vast_expression *
    xls_vast_localparam_ref_as_expression(struct xls_vast_localparam_ref *v);
    ...
    struct xls_vast_logic_ref *
    xls_vast_generate_loop_get_genvar(struct xls_vast_generate_loop *loop);
    ...
    void
    xls_vast_generate_loop_add_blank_line(struct xls_vast_generate_loop *loop);

    // Adds a comment inside the given generate loop.
    void xls_vast_generate_loop_add_comment(
        struct xls_vast_generate_loop *loop,
        struct xls_vast_comment *comment
    );
    ...
    bool xls_vast_generate_loop_add_always_comb(
        struct xls_vast_generate_loop *loop,
        struct xls_vast_always_base **out_always_comb,
        char **error_out
    );
    ...
    bool xls_vast_generate_loop_add_always_ff(
        ...
        size_t sensitivity_list_count,
        struct xls_vast_always_base **out_always_ff,
        char **error_out
    );
    ...
    // Adds a localparam item using an existing def inside the given generate
    // loop.
    struct xls_vast_localparam_ref *
    ...
    struct xls_vast_indexable_expression *
    xls_vast_index_as_indexable_expression(struct xls_vast_index *index);
    ...
    // Adds a non-blocking assignment statement (lhs <= rhs) to a statement
    ...
    // Emits an expression to a string and returns an owned C string; caller
    // must free with xls_c_str_free.
    ...
    // 'xls_vast_verilog_file_make_neg_edge'. Returns true on success. On
    // failure, returns false and sets error_out. The caller is responsible for
    // freeing error_out if it is not NULL.
    ...
    bool xls_vast_verilog_module_add_always_at(
        ...
        struct xls_vast_expression **sensitivity_list_elements,
        size_t sensitivity_list_count,
        struct xls_vast_always_base **out_always_at,
        char **error_out
    );
    ...
    bool xls_vast_verilog_module_add_logic(
        ...
        const char *name,
        struct xls_vast_data_type *type,
        struct xls_vast_logic_ref **out_logic_ref,
        char **error_out
    );
    ...
    struct xls_vast_expression *xls_vast_verilog_file_make_pos_edge(
        struct xls_vast_verilog_file *f,
        struct xls_vast_expression *signal_expr
    );
    ...
    struct xls_vast_statement_block *
    xls_vast_conditional_get_then_block(struct xls_vast_conditional *cond);
    ...
    struct xls_vast_statement_block *xls_vast_conditional_add_else_if(
        ...
        struct xls_vast_expression *expr_cond
    );

    // Adds an else clause (no condition) to the given conditional and returns
    // the associated statement block. Must be called at most once.
    struct xls_vast_statement_block *
    xls_vast_conditional_add_else(struct xls_vast_conditional *cond);
    ...
    // Adds a case item with the given match expression to the case statement
    // and returns the associated statement block for that item.
