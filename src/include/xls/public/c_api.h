...
extern "C"
{
    ...
    // default warning set.
    //
    // Precondition: if `warnings_out` is provided then `warnings_out_count`
    // must also be provided.
    //
    // Note that if the `warnings_out_count` is populated with zero (i.e. there
    // were no warnings) then we can return `nullptr` for `warnings_out`.
    bool xls_convert_dslx_to_ir_with_warnings(
        ...
        size_t enable_warnings_count,
        const char *disable_warnings[],
        size_t disable_warnings_count,
        bool warnings_as_errors,
        bool force_implicit_token_calling_convention,
        char ***warnings_out,
        size_t *warnings_out_count,
        char **error_out,
        char **ir_out
    );

    bool xls_convert_dslx_path_to_ir(
        const char *path,
        const char *dslx_stdlib_path,
        const char *additional_search_paths[],
        size_t additional_search_paths_count,
    ...
    // As above `xls_convert_dslx_to_ir_with_warnings`, but for a filesystem
    // path instead of DSLX text.
    ...
    bool xls_mangle_dslx_name_full(
        const char *module_name,
        const char *function_name,
        xls_calling_convention convention,
        const char *const free_keys[],
        size_t free_keys_count,
        const struct xls_dslx_parametric_env *param_env,
        const char *scope,
        char **error_out,
        char **mangled_out
    );
    ...
    bool xls_schedule_and_codegen_package(
        struct xls_package *p,
        const char *scheduling_options_flags_proto,
        const char *codegen_flags_proto,
    ...
    bool xls_parse_typed_value(
        const char *input,
        char **error_out,
        struct xls_value **xls_value_out
    );

    bool xls_value_make_ubits(
        int64_t bit_count,
        uint64_t value,
        char **error_out,
        struct xls_value **xls_value_out
    ...
    // Returns a new bits rope with the given bit count. The caller must free
    // the returned `bits_rope` via `xls_bits_rope_free` if it is non-null.
    ...
    void xls_bits_rope_append_bits(
        struct xls_bits_rope *bits_rope,
    ...
    // after all bits have been appended to the bits rope, i.e. the bits rope
    // must be "full".
    ...
    bool xls_value_get_kind(
        const struct xls_value *value,
        char **error_out,
        xls_value_kind *kind_out
    );

    bool xls_value_make_sbits(
        int64_t bit_count,
        int64_t value,
        char **error_out,
        struct xls_value **xls_value_out
    ...
    // Returns a new array-kind XLS value which the caller must free. There must
    // be at least one element and all provided elements must be of the same
    // type otherwise an error result is returned.
    bool xls_value_make_array(
    ...
    struct xls_value *
    xls_value_make_tuple(size_t element_count, struct xls_value **elements);

    // Returns a clone of the given value -- the caller must free the returned
    // value via `xls_value_free`.
    struct xls_value *xls_value_clone(const struct xls_value *value);

    // Returns the element at the given index in the value -- the value must be
    // an aggregate (i.e. tuple or array) or an error is returned.
    bool xls_value_get_element(
        const struct xls_value *value,
        size_t index,
        char **error_out,
        struct xls_value **element_out
    );
    ...
    bool xls_value_get_element_count(
        const struct xls_value *value,
        char **error_out,
        int64_t *count_out
    );

    // Attempts to extract a "bits" value from the given XLS value -- the
    // resulting `bits_out` is owned by the caller and must be freed via
    // `xls_bits_free()` on success.
    bool xls_value_get_bits(
        const struct xls_value *value,
        char **error_out,
    ...
    // `error_out` is populated with an error message and the caller must free
    // the `error_out` pointer with `xls_c_str_free`.
    bool xls_bits_make_bits_from_bytes(
        size_t bit_count,
        const uint8_t *bytes,
        size_t byte_count,
        char **error_out,
    ...
    // Helper routine for making an unsigned bits value using a value that fits
    // in a 64-bit word.
    //
    // `bit_count` must be large enough to hold the value or an error is returned.
    bool xls_bits_make_ubits(
        int64_t bit_count,
        uint64_t value,
        char **error_out,
        struct xls_bits **bits_out
    ...
    // As above but for a signed bit value -- `bit_count` must be large enough
    // to hold the value assuming sign extension or an error is returned.
    bool xls_bits_make_sbits(
        int64_t bit_count,
        int64_t value,
        char **error_out,
        struct xls_bits **bits_out
    ...
    // Equality compares exact bit patterns including bit_count. If widths
    // differ, equality returns false (no error). Pointers must be non-null.
    bool xls_bits_eq(const struct xls_bits *a, const struct xls_bits *b);

    // Inequality and comparison operations on bits values.
    // Unsigned comparisons interpret both operands as unsigned. Mixed widths
    // are allowed; comparisons follow bits_ops semantics. Pointers must be
    // non-null.
    ...
    // Signed comparisons interpret both operands as two's-complement signed
    // values. Mixed widths are allowed; comparisons follow bits_ops semantics.
    ...
    // Converts the given bits value to a signed or unsigned integer 64-bit
    // integer value. If the conversion is not possible, false is returned,
    // `error_out` contains the error message and the caller must free the
    // `error_out` pointer with `xls_c_str_free`.
    bool xls_bits_to_uint64(
        const struct xls_bits *bits,
        char **error_out,
        uint64_t *value_out
    );
    bool xls_bits_to_int64(
        const struct xls_bits *bits,
        char **error_out,
        int64_t *value_out
    );
    struct xls_bits *xls_bits_width_slice(
        const struct xls_bits *bits,
        int64_t start,
        int64_t width
    );

    struct xls_bits *xls_bits_shift_left_logical(
        const struct xls_bits *bits,
        int64_t shift_amount
    );

    struct xls_bits *xls_bits_shift_right_logical(
        const struct xls_bits *bits,
        int64_t shift_amount
    );

    struct xls_bits *xls_bits_shift_right_arithmetic(
        const struct xls_bits *bits,
        int64_t shift_amount
    );
    ...
    struct xls_bits *
    xls_bits_add(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_sub(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_and(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_or(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_xor(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_umul(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_smul(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_udiv(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_sdiv(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_umod(const struct xls_bits *lhs, const struct xls_bits *rhs);

    struct xls_bits *
    xls_bits_smod(const struct xls_bits *lhs, const struct xls_bits *rhs);
    ...
    bool xls_format_preference_from_string(
        const char *s,
        char **error_out,
    ...
    bool xls_value_to_string_format_preference(
        const struct xls_value *v,
        xls_format_preference format_preference,
        char **error_out,
        char **result_out
    );

    bool xls_bits_to_string(
        const struct xls_bits *bits,
        xls_format_preference format_preference,
    ...
    //
    // Note that in a tuple or array the earlier fields/members are stored in
    // the most significant bits of the result value.
    //
    // Note: the returned bits buffer is owned by the caller and must be freed
    // via `xls_bits_free`.
    ...
    //
    // e.g. `xls_convert_dslx_to_ir` gives back `ir_out` which can be
    // deallocated by this function.
    //
    // This function is primarily useful when the underlying allocator may be
    // different between the caller and the XLS library (otherwise the caller
    // could just call `free` directly).
    ...
    bool
    xls_function_to_string(const struct xls_function *f, char **string_out);

    // Returns the "top" (i.e. entry point) function base (i.e. function or
    // proc) of the given package.
    struct xls_function_base *xls_package_get_top(struct xls_package *p);

    // Sets the top function base of the given package to the function base with
    // the given name.
    bool xls_package_set_top_by_name(
        struct xls_package *p,
        const char *name,
        char **error_out
    );
    ...
    // `error_out` with an error message that must be freed via
    // `xls_c_str_free`.
    ...
    // freed with `xls_function_ptr_array_free`. The length of the array is
    // saved in `count_out`. If an error occurs, false is returned and
    // `error_out` is populated with an error message, which must be freed with
    // `xls_c_str_free`. Note: the returned functions do not need to be freed,
    // they are tied to the package's lifetime.
    bool xls_package_get_functions(
        struct xls_package *package,
        char **error_out,
        struct xls_function ***result_out,
        size_t *count_out
    ...
    // package's lifetime.
    bool xls_package_get_type_for_value(
        struct xls_package *package,
        struct xls_value *value,
        char **error_out,
        struct xls_type **result_out
    );

    // Returns the kind of the given type.
    bool xls_type_get_kind(
    ...
    // Note: the returned type does not need to be freed, it is tied to the
    // package's lifetime.
    bool xls_function_get_type(
        struct xls_function *function,
        char **error_out,
        struct xls_function_type **xls_fn_type_out
    );

    // Returns the name of the given function `function` -- `string_out` is
    // owned by the caller and must be freed.
    bool xls_function_get_name(
        struct xls_function *function,
        char **error_out,
        char **string_out
    );
    ...
    bool xls_function_get_param_name(
        struct xls_function *function,
        size_t index,
        char **error_out,
        char **name_out
    ...
    bool xls_function_type_to_string(
        struct xls_function_type *xls_function_type,
        char **error_out,
        char **string_out
    );
    ...
    bool xls_function_type_get_param_type(
    ...
    struct xls_type *
    xls_function_type_get_return_type(struct xls_function_type *type);

    // Interprets the given `function` using the given `args` (an array of size
    // `argc`) -- interpretation runs to a function result placed in
    // `result_out`, or `error_out` is populated and false is returned in the
    // event of an error.
    ...
    // AOT-compiles the given function and returns the resulting object code and
    // ABI metadata in newly allocated output buffers.
    //
    ...
    // Creates execution context required for direct invocation of AOT
    // entrypoints.
    //
    ...
    bool xls_aot_exec_context_create(
    ...
    void
    xls_aot_exec_context_clear_events(struct xls_aot_exec_context *context);
    ...
    int64_t xls_aot_entrypoint_trampoline(
        ...
        void *temp_buffer,
        struct xls_aot_exec_context *context,
    ...
    bool xls_aot_exec_context_get_trace_message(
        const struct xls_aot_exec_context *context,
        size_t index,
        char **error_out,
    ...
    // Runs the given `jit` function with the given `args` (an array of size
    // `argc`) and returns the result in `result_out`.
    //
    // Note:
    ...
    //   `xls_c_strs_free`.
    bool xls_function_jit_run(
        struct xls_function_jit *jit,
        size_t argc,
        const struct xls_value *const *args,
        char **error_out,
    ...
    // frees the array of  `xls_function` pointers -- the function should have
    // been allocated by the XLS library where ownership was passed back to the
    // caller.
    void
    xls_function_ptr_array_free(struct xls_function **function_pointer_array);

    void xls_trace_messages_free(
        struct xls_trace_message *trace_messages,
        size_t count
    );
    ...
    // function parameter and yields the function body expression (using
    // bit-vector operators).  For example, a two-parameter 32-bit adder becomes:
    ...
    bool xls_function_to_z3_smtlib(
        struct xls_function *function,
        char **error_out,
        char **string_out
    );
